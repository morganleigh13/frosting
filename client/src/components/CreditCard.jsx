import { setCheckoutForm, toggleCreditCard } from "../redux/authSlice";
import { useEffect } from "react";
import { useSelector, useDispatch } from "react-redux";
import { PiEyeClosedDuotone } from "react-icons/pi";
import { TbEye } from "react-icons/tb";

const CreditCard = ({ part, setLast4Digits, handleCreditCard }) => {
  const dispatch = useDispatch();
  const { user, authToken, showCardNumber } = useSelector(
    (state) => state.auth
  );
  const cardNumber = useSelector(
    (state) => state.auth.user.checkout?.payment?.cardNumber
  );

  return (
    <div className="cc-modal sweet">
      <div className="flex flex-col gap-5 p-5">
        {part === "first" && (
          <div className="credit-card-info--form">
            <div className="cc-input_container">
              <div class="input_container">
                <label className="cc-input_label" htmlFor="cc-password_field">
                  Card holder full name
                </label>
                <div className="relative w-full max-w-xs">
                  <input
                    value={user.checkout?.payment?.fullName}
                    onChange={(e) =>
                      dispatch(
                        setCheckoutForm({
                          checkout: {
              ...
              <div className="relative w-full max-w-xs">
                <input
                  value={user.checkout?.payment?.cardNumber}
                  onChange={(e) => {
                    ...
                    setLast4Digits(val.slice(-4));
                  }}
                  onBlur={(e) => handleCreditCard(e)}
                  className={`w-full rounded-md border text-base-content focus:outline-none transition ease-in-out duration-150 p-2 text-xl tracking-wider input ${
                    user.checkout?.error?.includes("creditCard")
                      ? "input-error border-4"
                      : "border-accent"
                  }`}
                  minLength={16}
                  maxLength={16}
                ...
                <button
                  type="button"
                  onClick={() => dispatch(toggleCreditCard())}
                  className="absolute inset-y-0 right-0 flex items-center pr-3 focus:outline-none"
                  aria-label={
                    showCardNumber ? "Hide card number" : "Show card number"
                  }
                >
                  {showCardNumber ? <PiEyeClosedDuotone /> : <TbEye />}
                </button>
              </div>
            </div>
            ...
            <div className="cc-input_container">
              ...
              <div className="flex gap-2">
                <input
                  value={user.checkout?.payment?.expires}
                  onChange={(e) =>
                    dispatch(
                      setCheckoutForm({
                        checkout: {
                ...
                />
                <input
                  value={user.checkout?.payment?.cvv}
                  onChange={(e) =>
                    dispatch(
                      setCheckoutForm({
                        checkout: {
        ...
        {part === "second" && (
          /* second half – name + last‑4 verify */
          <div className="credit-card-info--form">
            <div className="cc-input_container">
              <div class="input_container">
                <label className="cc-input_label" htmlFor="cc-password_field">
                ...
                <div className="relative w-full max-w-xs">
                  <textarea
                    value={user.checkout?.payment?.street}
                    onChange={(e) =>
                      dispatch(
                        setCheckoutForm({
                          checkout: {
                ...
                <div className="cc-input_container">
                  ...
                  <div className="flex gap-5">
                    <input 
                      value={user.checkout?.payment?.city}
                      onChange={(e) =>
                        dispatch(
                          setCheckoutForm({
                            checkout: {
                    ...
                    />
                    <select
                      value={user.checkout?.payment?.state}
                      onChange={(e) =>
                        dispatch(
                          setCheckoutForm({
                            checkout: {
                    ...
                    <input
                      value={user.checkout?.payment?.zip}
                      onChange={(e) =>
                        dispatch(
                          setCheckoutForm({
                            checkout: {
                      ...
                      placeholder="Zipcode"
                      type="text"
                      className="w-full border-accent rounded-md border text-base-content focus:bg-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150 p-2 mt-1 mb-1 text-xl tracking-wider"
                    />
                  </div>
                </div>
              </div>
